...
define([
	'kendo.backbone/collection.adapter',
	'kendo.backbone/backbone.transport',
	'kendo.backbone/backbone.client.transport'
], function (CollectionAdapter, BackboneTransport, BackboneClientTransport) {
	"use strict";
	...
	var DataSource = kendo.data.DataSource.extend({
		init: function (options) {
			...
			var colWrap = new CollectionAdapter(collection, this);

			// configure the Backbone transport with the collection
			var bbtrans = options.remoteDataSource ? 
				new BackboneTransport(colWrap) : new BackboneClientTransport(colWrap);
			_.defaults(options, {
				transport: bbtrans,
				serverFiltering: !!options.remoteDataSource,
				serverPaging: !!options.remoteDataSource,
				serverSorting: !!options.remoteDataSource
			});
